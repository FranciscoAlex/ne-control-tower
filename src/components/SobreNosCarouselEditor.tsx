...
export default function SobreNosCarouselEditor() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/sobre-nos-carousel-slides`)
      .then((r) => r.json())
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch(`${API_BASE}/sobre-nos-carousel-slides`, {
        method: 'PUT',
  ...
  const handleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/sobre-nos-carousel-slides/upload/${encodeURIComponent(slide.id)}`, {
        method: 'POST',
        body: form,
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  return (
    <Stack spacing={4}>
      <PageUrlBanner urls={{ path: '/sobre-nos', label: 'Sobre Nós — Carrossel de Destaque' }} />

      <Stack direction="row" alignItems="center" justifyContent="space-between">
        <Box>
          <Typography variant="h6" fontWeight={800} sx={{ color: '#1e293b' }}>
            Slides do Carrossel — Sobre Nós
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Gerir os banners da página Sobre Nós. Adicione botões e links para direcionar os utilizadores.
          </Typography>
        </Box>
        <Button
          variant="contained"
